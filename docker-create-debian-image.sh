...
declare -r IMAGE_TAG_SLIM=${IMAGE_TAG_SLIM:-"${IMAGE_TAG}"}
...
declare -r DEBOOTSTAP_TAR_SLIM="${BASE_DIR}/${IMAGE_REPOSITORY}-${IMAGE_TAG_SLIM}.tar"
...
if [[ "${SKIP_DOCKER_IMPORT}" == "false" ]] ; then
    ...
    rm "${DEBOOTSTAP_TAR}"
    docker import "${DEBOOTSTAP_TAR_SLIM}" "${IMAGE_REPOSITORY}:${IMAGE_TAG_SLIM}"
    rm "${DEBOOTSTAP_TAR_SLIM}"
fi
